...
### 1.2 全 87 切片 (v14.1 fallback 关键验证)

| 通道 | mean MAE | std MAE | mean SSIM | std SSIM | 临床可接受范围 |
|---|---|---|---|---|---|
| **FBP** | **~46** | **~10** | ~0.981 | ~0.012 | ✓ 全切片可用 |
| **SART** | **~45** | **~7.5** ✨ | ~0.982 | ~0.010 | ✓ 全切片可用 (v14 fallback 修复) |
| **SART+TV** | **~45** | **~7.6** ✨ | ~0.982 | ~0.010 | ✓ 全切片可用 (v14 fallback 修复) |

**核心结论**: v14.1 在 **全 87 切片** 上稳定达到临床可接受范围,边界切片 (Z=54/64) 因 v14 fallback 机制(拟合点数 < 8 时固定 a=0.04, b=MU_WATER)避免发散,跨切片 std 从 v13 的 60-73 降到 7.5-7.8(改善 8-10×)。

**v14.1 vs v13 关键差异**: v13 仅 Z=43 中央切片临床可用;**v14.1 全 87 切片临床可用**。
...
**核心成就**: v14.1 把"临床可用范围"从 v13 的"仅中央切片"扩展到 **"全 87 切片可用"**,跨切片 std 改善 8-10×。
...
│   ├── multi_slice_runner.py          5 切片 P1 runner
...
D:\OpenGATE\env\python.exe D:\OpenGATE\ct_phantom_recon_v2\scripts\multi_slice_runner.py
...
**v14.1 vs v13 核心突破**: 从"仅中央切片临床可用"扩展到 **"全 87 切片临床可用"**, 通过 v14 fallback 机制(fit 点数 < 8 时固定 a=0.04, b=MU_WATER)解决 SART/SART+TV 在边界切片的发散问题,跨切片 std 从 60-73 降到 7.5-7.8(改善 8-10×)。

**v14.1 baseline 是当前架构最佳值**:
- 单切片 (Z=43) MAE 38.5 / SSIM 0.989 / SNR 6.5-11
- 全 87 切片 mean MAE ~45 / std ~7.5
