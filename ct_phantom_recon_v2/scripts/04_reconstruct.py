...
# P1 多切片: 每个 z 用对应的 summary_z<Z>.json (默认 z=43 兼容旧调用)
SUMMARY = os.path.join(proj_dir, f"summary_z{int(os.environ.get('Z_IDX', 43)):03d}.json")

# ============= 算法选择 =============
RECON_ALG = "fbp"  # 默认 FBP; "sart" / "sart_tv" 切换

# P1 多切片: 模块顶层默认值 (main() 里会用 env 覆盖)
...
def main():
    ...
    sitk.WriteImage(sino_img, os.path.join(out_dir, f"sinogram_z{Z_IDX:03d}.mhd"))
    ...
    save_recon_as_mu(recon_fbp, os.path.join(out_dir, f"ct_recon_fbp_z{Z_IDX:03d}.mhd"))
    ...
    save_recon_as_mu(recon_sart, os.path.join(out_dir, f"ct_recon_sart_z{Z_IDX:03d}.mhd"))
    ...
    save_recon_as_mu(recon_tv, os.path.join(out_dir, f"ct_recon_sart_tv_z{Z_IDX:03d}.mhd"))
    ...
    with open(os.path.join(out_dir, f"summary_z{Z_IDX:03d}.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    # 检查重建
    try:
        check_recon_data(os.path.join(out_dir, f"ct_recon_fbp_z{Z_IDX:03d}.mhd"),
    ...
    try:
        ...
        for ax, img, title in zip(axes[1],
            ...
            plt.colorbar(im, ax=ax, fraction=0.046)
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"recon_compare_z{Z_IDX:03d}.png"), dpi=100)
        print(f"  recon_compare_z{Z_IDX:03d}.png saved")
