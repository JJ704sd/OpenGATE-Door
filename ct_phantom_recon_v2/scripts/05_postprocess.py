...
def mu_to_hu_with_mask_cal(mu, mask_2d, fov_radius_px=100,
    ...
    # v14 fallback: fit 点太少时退化为"固定 a/b"避免 a slope 发散
    # 触发场景: 边界切片 (Z=54/64) 器官覆盖 4-5 个, fit 点 6-7, a slope 发散 (0.5-1.2 vs 中央 0.033)
    # Fallback: 用固定 a=A_FALLBACK, b=MU_WATER, 跳过 lstsq, 保证 HU 范围临床合理
    n_fit_before_fallback = len(fit_points)
    use_fallback = (n_fit_before_fallback < FIT_MIN_THRESHOLD)
    if use_fallback:
        # 保留 organ_stats 信息但标记 fallback (用 dict 而非 str, 避免 main() 序列化崩)
        ...
        print(f"  ⚠ v14 fallback 触发: fit 点 {n_fit_before_fallback} < {FIT_MIN_THRESHOLD}, 用固定 a={A_FALLBACK}, b={MU_WATER} 跳过 lstsq")
        # 设置固定 a/b, 跳过 lstsq
        a = A_FALLBACK
        b = MU_WATER
        fit_points = []  # 清空, 表示走固定路径
        fit_weights = []


    # === 加权最小二乘 fit (正常路径) 或 fallback (固定 a/b) ===
    if use_fallback:
        # fallback 已在前面设好 a/b, 这里跳过 lstsq
        pass
    else:
