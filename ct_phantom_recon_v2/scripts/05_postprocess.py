...
A_FALLBACK = 0.04               # 接近 FBP 通道 a=0.033, 微调以匹配 SART μ 范围 (v14.2 P0-2 fix: fallback 改为物理公式 a=1, A_FALLBACK 暂保留)
A_MIN = 0.01                   # 物理意义: μ 衰减必须正向放大, 安全网 (v14.2: 删除后改走物理公式)
FIT_MIN_THRESHOLD = 8           # < 8 fit 点触发 fallback (中央 11 不触发, 边界 6-7 触发)
...
def mu_to_hu_with_mask_cal(mu, mask_2d, fov_radius_px=100,
    ...
    # v14.2 P0-2 fix: 自动检测高密度 anchor + 拒绝 air-zone outlier
    # 修复前 v11 anchor 把 mu_pred<0 的像素加入 lstsq, 把 a slope 拉反向 (v8 #1 失真共因根)
    if USE_AUTO_HIGH_DENSITY_ANCHOR and truth_hu_2d is not None:
        ...
        if anchor_res is not None:
            ...
            if mu_anchor_pred < 0:
                # Anchor 在 air zone (μ_offset<0 表示 less than air), 拒绝加入 fit
                # 物理意义: anchor 像素 HU 高但 μ 重建值低于空气, 是 SART/FBP 失真产生的"伪 anchor"
                print(f"  ⚠ anchor outlier rejected: mu_pred={mu_anchor_pred:.5f} < 0 (air zone, "
                      f"HU={anchor_hu:.1f}, n_voxels={n_anchor_voxels})")
                anchor_res = None
            else:
    ...
    # v14.2 P0-2 fix: fallback 用物理公式 (a=1, b=0)
    # 修复前: a=A_FALLBACK=0.04, b=MU_WATER=0.0195 给出 HU_air=0 而非 -1000 (压缩 4×)
    # 物理正确: HU = 1000 * (μ - μ_water) / μ_water, 不做 air 减法 (raw μ 已近 0)
    n_fit_before_fallback = len(fit_points)
    use_fallback = (n_fit_before_fallback < FIT_MIN_THRESHOLD)
    if use_fallback:
        organ_stats["__fallback__"] = {
            "method": "physical_v14_2",
            "n_fit_points": n_fit_before_fallback,
            "threshold": FIT_MIN_THRESHOLD,
        }
        print(f"  ⚠ v14.2 fallback 触发: fit 点 {n_fit_before_fallback} < {FIT_MIN_THRESHOLD}, "
              f"用物理公式 a=1, b=0 (air pixel HU=-1000, water pixel HU=0)")
        a = 1.0
        b = 0.0
        fit_points = []  # 标记走 fallback
        fit_weights = []
        use_mu_offset_formula = False  # fallback 改用 mu_cal = a*mu + b (raw mu)
    else:
        use_mu_offset_formula = True  # lstsq 路径保持原 mu_cal = a*mu_offset + b 公式


    # === 加权最小二乘 fit (正常路径) 或 fallback (固定 a/b) ===
    if not use_fallback:
        ...
        a = float(coef[0])
        b = float(coef[1])
        # 物理约束: a 转负是 fit 失真 (anchor 拒绝后若仍出现, 警告但不 clip — 让物理公式接管)
        if a < A_MIN:
            print(f"  ⚠ a={a:.4f} < A_MIN={A_MIN} (anchor 拒绝后仍异常), 改用物理公式 fallback")
            a = 1.0
            b = 0.0
            use_mu_offset_formula = False
        # 注意: v14.2 删除 v11 误导的 "b ≈ MU_WATER sanity check" —— 物理上 b 应接近 0
        # (a*mu_offset + b = mu_actual 当 mu_offset=0 → b = mu_actual_air = 0, 不应是 MU_WATER)

    # 应用校准公式
    if use_mu_offset_formula:
        mu_cal = a * mu_offset + b
    else:
        # fallback 物理公式: mu_cal = a*mu + b, 用 raw mu (不 offset)
        mu_cal = a * mu + b
...
def postprocess_one(name, in_path):
    ...
    if SCAN_MODE:
        ...
        out_path = os.path.join(out_dir, f"ct_post_{name}_z{ANCHOR_TRUTH_Z:03d}.mhd")
        img_out = sitk.GetImageFromArray(hu_denoise)
        img_out.SetSpacing(img.GetSpacing())
        img_out.SetOrigin(img.GetOrigin())
        sitk.WriteImage(img_out, out_path)
